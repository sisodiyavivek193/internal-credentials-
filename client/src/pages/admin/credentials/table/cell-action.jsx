...
const CellAction = ({ data, onRefresh }) => {
    ...
    const onDeleteConfirm = async () => {
        setLoading(true);
        try {
            await api.delete(`/admin/credentials/${data._id}`);
    ...
    const onRevokeConfirm = async () => {
        ...
        try {
            if (data.isActive) {
                await api.patch(`/admin/credentials/${data._id}/revoke`);
            } else {
                await api.patch(`/admin/credentials/${data._id}/restore`);
            }
    ...
    const navigateToUpdatePage = () => {
        router.push(`/admin/credential/${data._id}`);
    };
