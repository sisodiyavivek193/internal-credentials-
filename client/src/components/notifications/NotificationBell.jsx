    ...
    const togglePush = async () => {
        if (getPushPermission() === "denied") {
            toast.error("Notifications blocked", {
                description: "Browser mein notifications block hai. Address bar ke paas site settings icon se Notifications ko 'Allow' karo, phir page refresh karo.",
            });
            return;
        }
            ...
            if (pushEnabled) {
                ...
                if (!ok) {
                    toast.error("Enable failed", {
                        description: "Notification permission allow nahi hui. Popup mein 'Allow' select karna zaroori hai.",
                    });
                }
            }
        } finally {
                    ...
                    {!loading &&
                        notifications.map((n) => (
                            <DropdownMenuItem
                                ...
                                className={`group relative flex flex-col items-start gap-0.5 whitespace-normal py-2 pr-7 ${!n.isRead ? "bg-accent/60" : ""
                                    }`}
                            >
                                <button
